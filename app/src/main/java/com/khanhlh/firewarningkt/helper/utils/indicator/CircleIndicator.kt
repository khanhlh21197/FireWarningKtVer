package com.khanhlh.firewarningkt.helper.utils.indicator
...
class CircleIndicator : View {
    ...
    private fun handleTypedArray(context: Context, attrs: AttributeSet?) {
        ...
        mIndicatorRadius = typedArray.getDimensionPixelSize(
            R.styleable.CircleIndicator_ci_radius,
        ...
        val gravity = typedArray.getInt(
            R.styleable.CircleIndicator_ci_gravity,
    ...
    private fun setUpListener() {
        viewPager?.addOnScrollListener(object :
    ...
    private fun createMovingItem() {
        ...
        when (mIndicatorMode) {
            Mode.INSIDE -> paint.xfermode = PorterDuffXfermode(PorterDuff.Mode.SRC_ATOP)
            Mode.OUTSIDE -> paint.xfermode = PorterDuffXfermode(PorterDuff.Mode.SRC_OVER)
            Mode.SOLO -> paint.xfermode = PorterDuffXfermode(PorterDuff.Mode.SRC)
        }
    ...
    private fun startDrawPosition(containerWidth: Int): Float {
        if (mIndicatorLayoutGravity == Gravity.LEFT)
            return 0f
        val tabItemsLength =
            tabItems!!.size * (2 * mIndicatorRadius + mIndicatorMargin) - mIndicatorMargin
